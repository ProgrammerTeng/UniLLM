...
func NewModelsHandler(providerRepo *repository.ProviderRepo, db *gorm.DB) *ModelsHandler {
	return &ModelsHandler{providerRepo: providerRepo, db: db}
}

// ListModels returns only truly available models (OpenAI-compatible /v1/models).
...
// ModelCatalog returns public model details with pricing and capabilities (no auth).
...
// inferVendor determines the actual model vendor from the public model name.
